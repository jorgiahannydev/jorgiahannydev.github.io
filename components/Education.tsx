      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="flex flex-col gap-3">
          {items.map((item) => (
            <div
              key={item.title}
              className="bg-white rounded-xl border-l-4 border-pink-400 shadow-sm px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1"
            >
              <div>
                <h3 className="font-bold text-slate-800 text-sm">{item.title}</h3>
                <p className="text-pink-500 font-medium text-xs mt-0.5">{item.institution}</p>
              </div>
